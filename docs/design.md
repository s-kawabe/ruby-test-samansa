...
- **呼び出し元**: Apple
- **備考**: このエンドポイントは `webhook_logs` への保存とジョブのエンキューのみを行い、即時 200 OK を返す。`subscriptions` の更新や `subscription_events` への追記はバックグラウンドジョブ（Sidekiq）が非同期に処理する。この設計には2つの理由がある。
  - **Apple のタイムアウト対策**: Apple は一定時間内に 200 OK が返らない場合に同じ Webhook を再送する。DB 処理を同期的に行うと、負荷スパイク時にタイムアウトが発生してリトライが連発し、冪等性チェックが必要になる頻度が上がる。
  - **Webhook 急増時のスループット確保**: セール等で同時に大量の Webhook が届いても、API は `webhook_logs` への INSERT のみで即時応答し、重い DB 処理はジョブキューで順次捌く。これにより DB コネクションの枯渇を防ぎ、処理が詰まらない構造にする（要件: パフォーマンス NFR）。
...
Apple Webhook（経路: Apple サーバー → Rails サーバー）とクライアントの仮開始 API（経路: ユーザーデバイス → Rails サーバー）は並走する。Apple からのサーバー間通信は安定・高速な一方、クライアントからのリクエストはモバイル回線の不安定さ・アプリのバックグラウンド移行・StoreKit コールバックの処理時間などの影響を受けるため、Webhook が先に到達するケースは本番環境で実際に発生する。稀なケースではあるが「必ず起こるもの」として設計し、どちらの順序でも最終的な状態が一致するよう Upsert で対処する。
